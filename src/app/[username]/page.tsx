/**
 * GET /<username> — the vanity URL, resolved to the real profile page.
 *
 ...
 *
 * This was a `route.ts` returning `new Response("Not found")`, and that plain
 * black line of text was what every mistyped URL in the app rendered — because
 * a single dynamic segment here catches EVERY unknown top-level path, and a
 * route handler returns bytes, not UI. It can never reach `app/not-found.tsx`.
 * A page can: `notFound()` renders the nearest not-found boundary, which is the
 * designed 404. The username lookup below is unchanged — the handler's job was
 * always "does this account exist", and it still is. Only the shape of the
 * answer changed.
 *
 * ── Route precedence ─────────────────────────────────────────
 *
 ...
 * it did and never reaches this page. Route groups do not appear in the URL,
 ...
 * ── Why 308 ──────────────────────────────────────────────────
 *
 * `permanentRedirect` (308), not `redirect` (307): permanent, and method- and
 * body-preserving. The canonical URL of a profile is `/profile/<username>` —
 * that is what the page's `alternates.canonical` says and what the sitemap
 * lists — so this must not look to a crawler like a second address for the same
 * content.
 */
...
/** Rebuilds `?a=1&b=2` so a query string on the vanity link survives the hop. */
...
export default async function VanityProfilePage({
  ...
  // Does this account exist? Users first: they are the ones the card prints a
  // vanity URL for, so the common case is one round trip. `/public/profile/`
  // 404s for an organization rather than returning its bundle, so an org costs
  // the second call — and `type` on whichever bundle comes back is what picks
  // the destination.
  const bundle =
    (await getPublicUserProfile(username)) ??
    (await getPublicOrganizationProfile(username))

  // Not found, and deliberately indistinguishable from a typo — the same rule
  // the card renderer follows, for the same reason. A hidden profile and a
  // username nobody has ever registered must look identical from out here, or
  // this route becomes a way to enumerate which accounts exist behind the
  // public toggle. Both land on the same 404 page.
  if (!bundle?.profile?.username) notFound()

  permanentRedirect(
    `${profilePath(bundle.profile.username, bundle.type)}` +
      queryString(await searchParams)
  )
}
